...
function buildHtml(templateType: string, data: Record<string, unknown>): { subject: string; html: string } {
  if (templateType === 'atleta_confirmacao') {
    const valueRows = data.baseAmount
      ? `${row('Inscrição', `R$ ${data.baseAmount}`)}${data.discountAmount && Number(data.discountAmount) > 0 ? row(`Cupom ${data.couponCode ?? ''}`, `- R$ ${data.discountAmount}`) : ''}${row('Taxa da plataforma', `R$ ${data.platformFee}`)}${row('Total pago', `R$ ${data.amount}`)}`
