...
const FROM_EMAIL = 'noreply@022runners.com.br'
...
const HEADER = '<div style="background:#000;padding:24px;text-align:center;border-bottom:3px solid #C9A84C;"><h1 style="color:#C9A84C;font-size:28px;font-weight:900;margin:0;letter-spacing:2px;">022RUNNERS</h1><p style="color:#94a3b8;font-size:12px;margin:4px 0 0;">RUNNING COMMUNITY — REGIÃO DOS LAGOS RJ</p></div>'
const FOOTER = '<div style="background:#f9fafb;padding:16px;text-align:center;border-top:1px solid #e5e7eb;"><p style="color:#9ca3af;font-size:11px;margin:0;">022runners.com.br · São Pedro da Aldeia, RJ</p><p style="color:#9ca3af;font-size:11px;margin:4px 0 0;">Dúvidas? WhatsApp: (22) 97404-4125</p></div>'
...
function buildHtml(templateType: string, data: Record<string, unknown>): { subject: string; html: string } {
  ...
  if (templateType === 'atleta_lembrete') {
    return {
      ...
      html: `<div style="${BASE_STYLE}">${HEADER}<div style="padding:32px 24px;"><p style="color:#374151;font-size:16px;margin:0 0 16px;">Olá, <strong>${data.athleteName}</strong>!</p><p style="color:#374151;font-size:15px;margin:0 0 24px;">Faltam <strong>3 dias</strong> para o seu evento!</p><div style="background:#fffbeb;border:2px solid #C9A84C;border-radius:12px;padding:20px;text-align:center;margin:0 0 24px;"><p style="color:#92400e;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:2px;margin:0 0 6px;">Número de Peito</p><p style="color:#C9A84C;font-size:48px;font-weight:900;font-family:monospace;margin:0;">#${data.registrationNumber}</p></div><table style="width:100%;border-collapse:collapse;">${row('Evento', String(data.eventTitle))}${row('Data', String(data.eventDate))}${row('Largada', String(data.eventTime))}${row('Local', String(data.eventCity))}</table></div>${FOOTER}</div>`,
    }
  }
