...
function supabaseToEvent(e: any): Event {
  const distances = (e.distances || []).map((d: any, i: number) => ({
    id: `d${i}`, name: d.name, distanceKm: parseFloat(d.name) || 0, price: Number(d.price),
  }));
  return {
    id: e.id, name: e.title, subtitle: e.description?.slice(0, 80) || '',
    ...
    organizerId: e.organizer_id || '', slug: e.slug,
  };
}
